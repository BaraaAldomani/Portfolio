<x-layout :meta="$meta">
    ...
    <section class="relative">
        <div class="mx-auto grid max-w-7xl items-center gap-12 px-5 py-16 sm:px-8 sm:py-24 lg:grid-cols-[1.1fr_0.9fr]">
            <div data-load>
                ...
                <p data-load-item class="mt-5 text-sm font-bold tracking-wide text-primary-700">{{ __('common.brand') }} · {{ __('v3.role') }}</p>
                <h1 data-load-item class="mt-3 text-4xl font-extrabold leading-[1.08] tracking-tight text-ink sm:text-5xl lg:text-6xl">{{ __('v3.hero.title') }}</h1>
                <p data-load-item class="mt-6 max-w-xl text-lg leading-relaxed text-muted">{{ __('v3.hero.lead') }}</p>
                <div data-load-item class="mt-8 flex flex-wrap items-center gap-3">
                    <a href="{{ route('contact') }}" class="js-magnetic btn-press glow-cyan inline-flex items-center justify-center rounded-full bg-primary px-7 py-3.5 text-sm font-bold text-[#08121b] transition-shadow hover:bg-primary-400">{{ __('v3.hero.primary') }}</a>
            ...
            <div data-load>
                <div data-load-item class="v3-frame relative mx-auto max-w-sm lg:mx-0 lg:ms-auto">
                    <div data-img-reveal class="v3-photo-flip overflow-hidden rounded-2xl">
                        <img src="{{ asset(config('portfolio.images.portrait')) }}" alt="{{ __('common.brand') }}, {{ __('v3.role') }}"
                    ...
                    <div class="glass absolute -bottom-5 rounded-2xl p-4 shadow-lg ltr:-start-5 rtl:-end-5">
                        <p class="text-2xl font-extrabold text-primary-700"><span data-counter="4">0</span><span aria-hidden="true">+</span></p>
                        <p class="mt-0.5 text-xs font-semibold leading-snug text-muted">{{ __('v3.metrics.0.label') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="border-y border-line">
        <div class="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-20">
            <div class="reveal max-w-2xl">
                <h2 class="text-2xl font-extrabold text-ink sm:text-3xl">{{ __('v3.proof.title') }}</h2>
                <p class="mt-3 leading-relaxed text-muted">{{ __('v3.proof.lead') }}</p>
            </div>
            <dl class="mt-12 grid grid-cols-2 gap-5 sm:grid-cols-3" data-stagger>
                @foreach (__('v3.metrics') as $m)
                    <div class="reveal glass glass-hover rounded-2xl p-6 text-center">
                        ...
                        <dd class="mt-1 text-sm font-semibold text-muted">{{ $m['label'] }}</dd>
                    </div>
                @endforeach
            </dl>
        </div>
    ...
    <section class="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-24">
        <div class="reveal max-w-xl">
            <h2 class="text-2xl font-extrabold text-ink sm:text-3xl">{{ __('v3.focus.title') }}</h2>
            <p class="mt-3 leading-relaxed text-muted">{{ __('v3.focus.lead') }}</p>
        </div>
        <div class="mt-12 grid items-start gap-6 md:grid-cols-3" data-stagger>
            @foreach (__('v3.focus.items') as $item)
                <div class="reveal glass glass-hover group rounded-2xl p-7 {{ $loop->index === 1 ? 'glass-focus md:-mt-3 md:mb-3' : '' }}">
                    ...
                    <h3 class="mt-4 text-lg font-extrabold text-ink transition-colors duration-200 group-hover:text-primary-700">{{ $item['title'] }}</h3>
    ...
    <section class="border-t border-line">
        <div class="mx-auto max-w-6xl px-5 py-16 sm:px-8 sm:py-24">
            <div class="flex flex-wrap items-end justify-between gap-6">
                <div class="reveal max-w-xl">
                    <h2 class="text-2xl font-extrabold text-ink sm:text-3xl">{{ __('home.projects.title') }}</h2>
                    <p class="mt-3 leading-relaxed text-muted">{{ __('home.projects.lead') }}</p>
                </div>
