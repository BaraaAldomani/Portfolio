<x-layout :meta="$meta">
    ...
    <x-section bg="dark" spacing="hero">
        <div class="mx-auto max-w-3xl text-center">
            ...
            <p class="reveal mt-5 text-sm font-bold tracking-wide text-accent-300">{{ __('services.eyebrow') }}</p>
            <h1 data-reveal-text class="mt-3 text-4xl font-extrabold leading-[1.15] text-ink-on-dark sm:text-5xl">
                {{ __('services.title') }}
            </h1>
            <p class="reveal mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-muted-on-dark">{{ __('services.lead') }}</p>
    ...
    @php
        // Per-service icon chips (presentational only; keyed to services.items).
        $serviceIcons = [
    ...
    <x-section id="offer">
        <x-section-heading
            :eyebrow="__('services.offer.eyebrow')"
            :title="__('services.offer.title')"
            :lead="__('services.offer.lead')"
                    ...
                    <div data-parallax data-parallax-speed="0.05" class="{{ $loop->even ? 'lg:order-2' : '' }}">
                        <div class="reveal">
                            <div class="group js-tilt glow-border relative overflow-hidden rounded-3xl border border-line bg-surface-muted shadow-xl transition-shadow duration-500 hover:shadow-2xl">
                                <img src="{{ asset(config('portfolio.images.services.' . $key)) }}"
                    ...
                    <div data-stagger>
                        <div class="reveal-start flex items-center gap-3">
                            <span class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-primary-50 text-primary-700 ring-1 ring-inset ring-primary-100" aria-hidden="true">
                                <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">{!! $serviceIcons[$key] ?? $serviceIcons['systems'] !!}</svg>
                            </span>
                            <span class="text-sm font-bold text-accent-600">{{ $service['tag'] }}</span>
                        </div>
                        <h2 class="reveal-start mt-4 text-2xl font-extrabold text-ink sm:text-3xl">{{ $service['title'] }}</h2>
                        <p class="reveal-start mt-4 text-lg leading-relaxed text-muted">{{ $service['description'] }}</p>
    ...
    <x-section bg="dark">
        <div class="text-center">
            <p class="reveal text-sm font-bold tracking-wide text-accent-300">{{ __('services.stats.eyebrow') }}</p>
            <h2 class="reveal mt-2 text-2xl font-extrabold text-ink-on-dark sm:text-3xl">{{ __('services.stats.title') }}</h2>
        </div>

        <dl class="mt-14 grid grid-cols-2 gap-6 lg:grid-cols-4" data-stagger>
            @foreach (__('services.stats.items') as $stat)
                <div class="reveal rounded-2xl border border-line-on-dark bg-surface-dark-soft p-6 text-center">
                    ...
                    <dt class="mt-2 text-xs font-semibold text-muted-on-dark sm:text-sm">{{ $stat['label'] }}</dt>
                </div>
            @endforeach
        </dl>
    </x-section>
    ...
    <x-section bg="muted">
        <div class="text-center">
            <p class="reveal text-sm font-bold tracking-wide text-primary-700">{{ __('services.process.eyebrow') }}</p>
            <h2 class="reveal mt-2 text-2xl font-extrabold text-ink sm:text-3xl">{{ __('services.process.title') }}</h2>
        </div>
        ...
        <div class="relative mt-16">
            ...
            <ol class="relative grid gap-10 sm:grid-cols-2 lg:grid-cols-4" data-stagger>
                @foreach (__('services.process.steps') as $i => $step)
                    <li class="reveal text-center">
                        ...
                        <h3 class="mt-5 text-lg font-extrabold text-ink">{{ $step['title'] }}</h3>
                        <p class="mt-2 text-sm leading-relaxed text-muted">{{ $step['blurb'] }}</p>
                    </li>
                @endforeach
            </ol>
        </div>
    ...
    <x-section spacing="tight">
        @php($tech = __('services.tech'))
        <p class="reveal text-center text-sm font-bold tracking-wide text-muted">{{ __('services.tech_title') }}</p>
        <div class="js-marquee mt-8">
            <div class="js-marquee__track gap-3 py-1">
                @foreach ($tech as $t)
                    <span class="inline-flex items-center whitespace-nowrap rounded-full border border-line bg-surface px-5 py-2.5 text-sm font-bold text-ink">{{ $t }}</span>
                @endforeach
                @foreach ($tech as $t)
                    <span class="inline-flex items-center whitespace-nowrap rounded-full border border-line bg-surface px-5 py-2.5 text-sm font-bold text-ink" aria-hidden="true">{{ $t }}</span>
                @endforeach
            </div>
        </div>
    </x-section>
    ...
    <x-section bg="dark" width="narrow">
        <div class="text-center">
            <h2 class="reveal gradient-text text-3xl font-extrabold sm:text-4xl">{{ __('services.cta.title') }}</h2>
            <p class="reveal mx-auto mt-4 max-w-xl text-lg leading-relaxed text-muted-on-dark">{{ __('services.cta.lead') }}</p>
